    ...
    def clusterStats(self, locs3d_seq_arr):
        ...
        for l in [i for i in unique_labels if i!=-1]:        
            ...
            if (len(clust_xyz)>3):
                ...
                cluster_hull_spoints = hull.simplices
                pdist_ndarray = distance.pdist(cluster_hull_spoints)
                clust_hull_feret = pdist_ndarray.max()
            else:
            ...
            # Get the feret distance of the cluster. 
            clust_feret_dict[l] = clust_hull_feret 


        # Return the 3d localization sequence.            
        return clust_cent_dict, clust_size_dict, clust_area_dict, clust_vol_dict, clust_feret_dict          
