...
class TileSequencer(object):
    ...
    def locs3dSequence(self, tile_num_seq, locs_3d_directory, mol_list):
        ...
        for j in range(len(tile_num_seq)):
            ...
            if mol_list: locs3d_file_name = locs_3d_directory + str(tile_num) + ".data"
            
            else: locs3d_file_name = locs_3d_directory + str(tile_num) + ".data"
            ...
            if locs3d.shape[0] == 0: 
                print("skip empty tile")
            else:
                if locs3d.ndim == 1:
                    locs3d = np.reshape(locs3d,(1,3))
                
                locs_3d_seq = np.concatenate((locs_3d_seq,locs3d))
        if locs3d.shape[0] > 1:
            locs_3d_seq = locs_3d_seq[1:,:]
        return locs_3d_seq
