...
def locsFromTiles(expfolder, storm_exp_name, tile_size, num_locs_output, num_locs_est_lin_fit_output, num_locs_est_quad_fit_output, max_processes, storm_image_scale, tiles_df, locsPerTile2, uint8, roi, clust_pix_list_full_file):
    
    
    if uint8: data_directory_str = "chenghang_list_" + "tilesize_" + str(tile_size) + roi + "\\"
    ...
    if not os.path.exists(expfolder + "ML_result_750\\"  + data_directory_str + "num_locs\\"):
        os.mkdir(expfolder + "ML_result_750\\"  + data_directory_str + "num_locs\\")

    num_locs_directory = expfolder + "ML_result_750\\"  + data_directory_str + "num_locs\\"
    
    if not os.path.exists(expfolder + "ML_result_750\\"  + data_directory_str + "num_locs_estimate\\"):
        os.mkdir(expfolder + "ML_result_750\\"  + data_directory_str + "num_locs_estimate\\")
        
    num_locs_est_directory = expfolder + "ML_result_750\\"  +data_directory_str + "num_locs_estimate\\"        
    ...
    for img_num in tiles_df["Num_image"].unique():
        ...
        h5_file =   molecule_lists_directory + storm_exp_name + "_" + "%03d"%(img_num-1) + ".hdf5"     

        # Get file for dictionary of localizations from all tiles in tiles_list for given image number.        
        locs_dict_directory = expfolder + "ML_result_750\\"  + "locs_dictionary_tilesize_" + str(tile_size) + "_ROIs\\"
        locs_dict_file_name = locs_dict_directory + "locs_dict_img_" + str(img_num) + ".data"        
        
        if uint8: locs_est_dict_directory = expfolder + "ML_result_750\\"  + "chenghang_list_tilesize_" + str(tile_size) + "_sorted\\num_locs_estimate"
