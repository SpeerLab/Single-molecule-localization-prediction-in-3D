...
size_filter = 1 # each protein molecule: 1 * 2 * 4 * 2
outpath = 'X:\\Chenghang\\4_Color\\Raw\\1.11.2021_B2P8C_B_V2\\ML_result_750_neg\\'
pathname = outpath + 'gtgt_cluster_stats_sequences_eps_20_min_smpl_6\\'
...
Num_of_sub.to_csv(outpath+'Gtgt_Num_of_sub.csv',index = False)
...
Sub_Size.to_csv(outpath+'Gtgt_Sub_size.csv',index = False)
...
Sub_volume.to_csv(outpath+'Gtgt_Sub_volume.csv',index = False)
...
Cluster_ID.to_csv(outpath+'Gtgt_Cluster_ID.csv',index = False)
...
Sub_X.to_csv(outpath+'Gtgt_Sub_X.csv',index = False)
...
Sub_Y.to_csv(outpath+'Gtgt_Sub_Y.csv',index = False)
...
Sub_Z.to_csv(outpath+'Gtgt_Sub_Z.csv',index = False)
