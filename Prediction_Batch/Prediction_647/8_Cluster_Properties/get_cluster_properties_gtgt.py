...
#cl_stats_directory = expfolder + "gtgt_stats_sequences_eps_" + str(eps) + "_min_smpl_" + str(min_samples) + "\\"
...
csv_filename = expfolder + csv_name_handle[-2] + '_subcluster_gtgt.csv'
...
# Iterate over sequences.
    for file in files:
        ...
        for key in cl_sz_dict.keys():
            ...
            cur_csv_row = [str(sub_clust_count),str(clust_count),str(cur_cetr_post[0]),str(cur_cetr_post[1]),str(cur_cetr_post[2]),str(cl_sz_dict[key]),str(cl_vl_dict[key]),str(cl_ar_dict[key])]
...
csv_filename_sub_num_incluster = expfolder + csv_name_handle[-2] + '_subcluster_num_gtgt.csv'
with open(csv_filename_sub_num_incluster,'w',newline='') as csvfile:
    csvwriter = csv.writer(csvfile)
    csvwriter.writerow(sub_cl_num)
