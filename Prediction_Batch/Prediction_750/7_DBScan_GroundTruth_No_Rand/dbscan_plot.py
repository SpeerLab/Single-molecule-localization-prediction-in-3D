...
expfolder = "Y:\\Chenghang\\04_4_Color\\Exp_Group\\1.2.2021_P2EA_B_V2\\ML_result_750_pos\\"
...
experiment_directory = expfolder
...
if mol_list: 
    ...
    if randm:
        ...
        dbscan_seq_directory = experiment_directory + "gtgt_dbscan_output_sequences_eps_" + str(eps) + "_min_smpl_" + str(min_samples) + "\\"     
...
if mol_list: 
    ...
    if randm:
        ...
        locs_sequences_directory = experiment_directory + "locs3d_gtgt_sequences\\"    
...
for i in range(seq_start, seq_end+1):
    ...
    if cluster_3d:
        ...
        for k, col in zip(unique_labels, colors):
            ...
            noise_mask = labels == -1
            if locs3d_seq_arr.shape[0] > 3:
                xy = locs3d_seq_arr[noise_mask] 
            else:
                continue;
            ax.scatter(xy[:, 0], xy[:, 1], xy[:, 2], c=tuple(col), s=2,linewidth=0, alpha=1) 
            ...
            if (sum(class_member_mask) > clst_sz_fltr):
                ...
                # Plot x,y,z coordinates for clusters.                
                # ax = plt.axes(projection='3d')
                ax.scatter(xy[:, 0], xy[:, 1], xy[:, 2], c=tuple(col), s=10,linewidth=0, alpha=1) 
