...
"""
"""
For the test, didn't use filtered ROIs, used all ROIs including small ones.
It seems like tensorflow can't recognize all gpus properly. To get it work I need CUDA,
which requires a graphic card driver version, which is currently too old.

You might sometimes need to go line 225 to change the name of the model.

IMPORTANT: use cmd to run the training rather than IDLE or other notebooks. Training of each epoch should
take less than 10 seconds. Batch size of 128 takes ~70% GPU usage. 
"""
...
training_data_directory = data_directory + "chenghang_list_tilesize_72_uint8_ROIs_from_shuffled\\"
...
data_directory_str = "chenghang_list_" + "tilesize_" + str(tile_size) + "_uint8_ROIs_from_shuffled\\"

tiles_data_path = data_directory + data_directory_str + "tiles\\"
num_locs_data_path = data_directory + data_directory_str + "num_locs\\"
num_locs_est_data_path = data_directory + data_directory_str + "num_locs_estimate\\"
...
#add: convert float64 to float32 for tiles_train.
tiles_train = np.float32(tiles_train)
