...
def locsFromTiles(expfolder, storm_exp_name, tile_size, num_locs_output, num_locs_est_lin_fit_output, num_locs_est_quad_fit_output, max_processes, storm_image_scale, tiles_df, locsPerTile2, uint8, roi, clust_pix_list_full_file):
    ...
    if not os.path.exists(expfolder + "make_data\\" + data_directory_str):
        os.mkdir(expfolder + "make_data\\" + data_directory_str)

    if not os.path.exists(expfolder + "make_data\\" + data_directory_str + "num_locs\\"):
        os.mkdir(expfolder + "make_data\\" + data_directory_str + "num_locs\\")

    num_locs_directory = expfolder + "make_data\\" + data_directory_str + "num_locs\\"
    
    if not os.path.exists(expfolder + "make_data\\" + data_directory_str + "num_locs_estimate\\"):
        os.mkdir(expfolder + "make_data\\" + data_directory_str + "num_locs_estimate\\")
        
    num_locs_est_directory = expfolder + "make_data\\" +data_directory_str + "num_locs_estimate\\"        
    ...
    for img_num in tiles_df["Num_image"].unique():
        ...
        locs_dict_directory = expfolder + "make_data\\" + "locs_dictionary_tilesize_" + str(tile_size) + "_ROIs\\"
        locs_dict_file_name = locs_dict_directory + "locs_dict_img_" + str(img_num) + ".data"        
        
        if uint8: locs_est_dict_directory = expfolder + "make_data\\" + "locs_estimate_dictionary_uint8_tilesize_" + str(tile_size) + "_ROIs\\"
